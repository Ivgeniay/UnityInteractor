...
namespace InteractionSystem
{
    public sealed partial class Sequence
    {
        public class SequenceMaster
        {
            ...
            public void Append(BaseInteractionAction sequence)
            {
                if (!Sequences.Contains(sequence))
                    Sequences.Add(sequence); 
            }
            public void Remove(BaseInteractionAction sequence)
            {
                if (Sequences.Contains(sequence))
                    Sequences.Remove(sequence); 
            }
            public void Remove(INode action)
            {
                if (action is BaseInteractionAction e)
                {
                    if (Sequences.Contains(e)) Sequences.Remove(e);
                } 
            }
